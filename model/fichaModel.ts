...
export class Ficha {
  ...
  public async actualizarFicha(): Promise<{success: boolean, message: string, ficha?: Record<string,unknown>}> {
      ...
      if (!this._objFicha || !this._objFicha.idficha) {
        throw new Error("Objeto no valido o Id no valido")
      } 

      const {codigo, fecha_inicio_lectiva, fecha_fin_lectiva, fecha_fin_practica, programa_idprograma,idficha} = this._objFicha;

      if (!idficha||!codigo || !fecha_inicio_lectiva || !fecha_fin_lectiva || !fecha_fin_practica || !programa_idprograma == null) {
        throw new Error("Faltan campos requeridos")
      }

      await Conexion.execute("START TRANSACTION")
      const sql = await Conexion.execute("UPDATE ficha SET codigo =?,fecha_inicio_lectiva=?,fecha_fin_lectiva=?,fecha_fin_practica=?,programa_idprograma=? WHERE idficha=?",
        [codigo, fecha_inicio_lectiva, fecha_fin_lectiva, fecha_fin_practica, programa_idprograma,idficha],
      );
      
      if (sql && typeof sql.affectedRows === "number" && sql.affectedRows>0) {
        const [fichaActualizada] = await Conexion.query(
